...
function problem1(input: string) {
    const wireInstructions = parseInput1(input)
    const wire1 = makeWire(wireInstructions[0])
    const wire2 = makeWire(wireInstructions[1])
    const intersections = new Set(Array.from(wire1).filter(p => wire2.has(p)));
    intersections.delete(ps([0,0]))

    return findNearestIntersectionDistance(intersections)
}

function parseInput1(input: string): Instruction[][] {
    const is = input.trim().split('\n').map(l =>
        l.trim().split(',').map(i => {
            const i2 = i.trim()
            let instruction: Instruction = {
                dir: i2[0] as Dir,
                distance: parseInt(i2.substring(1))
            }
            return instruction
        })
    )
...
function ps(pos: number[]) {
    return pos.join('_')
}

function sp(str: string) {
    return str.split('_').map(s => parseInt(s))
}

function setPoints(currPos: number[], wi: Instruction, points: Set<string>) {
    const dirToVelocities = {
        'R': [1, 0],
        'L': [-1, 0],
        'U': [0, -1],
        'D': [0, 1]
    }
    let [vx, vy] = dirToVelocities[wi.dir]
    for (let i = 0; i < wi.distance; i++) {
        currPos[0] += vx
        currPos[1] += vy
        points.add(ps(currPos))
    }
}

function makeWire(wireInstructions: Instruction[]): Set<string> {
    let points = new Set<string>()
    let currPos = [0, 0]
    points.add(ps(currPos))

    wireInstructions.forEach(wi => {
        setPoints(currPos, wi, points)
    })

    return points
}

function findNearestIntersectionDistance(intersections: Set<string>) {
    const distances = Array.from(intersections).map(p => sp(p)).map(p => Math.abs(p[0]) + Math.abs(p[1]))
